...
if lower_abs < 0:  # Caso onde faixa de valores passa negativamente de 0
    ...
    mask_abs = cv2.inRange(  # Mascara da faixa excecao
        hsv_image, (lim_abs, 0, 0), (179, 255, 255))
    hsv_image[mask_abs > 0, 0] = \
        hsv_image[mask_abs > 0, 0] - 90  # Inverter faixa excecao
    hsv_image[mask_aux > 0, 0] = \
        hsv_image[mask_aux > 0, 0] + 90  # Inverter faixa padrao
    ...
    mask_abs = cv2.inRange(  # Mascara da faixa excecao
        hsv_image, (0, 0, 0), (lim_abs, 255, 255))
    hsv_image[mask_abs > 0, 0] = \
        hsv_image[mask_abs > 0, 0] + 90  # Inverter faixa excecao
    hsv_image[mask_aux > 0, 0] = \
        hsv_image[mask_aux > 0, 0] - 90  # Inverter faixa padrao
